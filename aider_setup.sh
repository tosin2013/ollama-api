...
# Function to set Ollama API base
set_ollama_base() {
    update_model_keys_conf "ollama-model" "OLLAMA_API_BASE" "http://127.0.0.1:11434"
}

# Function to pull and serve Ollama model
setup_ollama() {
    model=$1
    ollama pull $model
    ollama serve &
...
start_aider() {
    ...
    if [ ! -f "$model_keys_file" ]; then
        echo "Model configuration file not found."
        exit 1
    fi
